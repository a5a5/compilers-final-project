...
(define prims-list '(= > < <= >= + - * / 
                     ...
                     eq? eqv? equal? not
                     string string->list string-ref substring string-append string-length;added string functions
                     ))
(define (prefix x)
...
(define (simplify-ir ir-e)
  (define (T e)
    (match e
           ...
           [`(prim string ,cs ...)
            `(apply-prim string ,(T `(prim list ,@cs)))]

           [`(prim string-append ,ss ...)
            ''""]
           [`(prim string-append ,ss)
            (T ss)]

           [`(prim string-append ,s1 ,s2 ,s3 ,r ...)
            (T `(prim string-append ,s1 (prim string-append ,s2 ,s3 ,@r)))]
...
(define (eval-llvm llvm-str)
  ...
  (when (not recent-header)
        ...
        (system (string-append clang++-path " header-gc.cpp " " -S -emit-llvm -o header.ll")))
  ...
  (system (string-append clang++-path " combined.ll " " bdwgc/gc.a " " -o bin"))
