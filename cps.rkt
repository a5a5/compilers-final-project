...
;also add x into lam-set (which is gonna have to be a functional set :(
...
;check that if fae is a symbol, then it is in the lam-set
           [`(,fae ,args ...)
            (define co (gensym 'co))
            (define msg (gensym 'msg))
            (define err (gensym 'err))
            #;`(let ([,co (prim procedure? ,(T-ae fae))])
               (if ,co
                   (,(T-ae fae) ,cae ,@(map T-ae args))
                   (let ([,msg '"E"]) (let ([,err (prim halt ,msg)]) (,err ,cae ,msg)))
                   )
               )
            `(,(T-ae fae) ,cae ,@(map T-ae args))
            ]
